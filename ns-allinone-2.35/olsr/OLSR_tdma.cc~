 ...
 * $Id: olsr-tdma.cc,v 1.18 2011/10/02 22:32:34 tom_henderson Exp $
 *
...
//
// $Header: /cvsroot/nsnam/ns-2/olsr/olsr-tdma.cc,v 1.18 2011/10/02 22:32:34 tom_henderson Exp $
//
// olsr-tdma.cc
...
static OLSR_MIB OMIB = {
	OLSR_SlotTime, OLSR_CCATime,
	OLSR_RxTxTurnaroundTime, OLSR_SIFSTime, OLSR_PreambleLength,
	OLSR_PLCPHeaderLength
};	

/* Timers */
void OlsrTdmaTimer::start(Packet *p, double time)
{
...
void OlsrTdmaTimer::stop(Packet *p) 
{
	Scheduler &s = Scheduler::instance();
	assert(busy_);
...
/* ======================================================================
   ...
   ====================================================================== */
static class OlsrTdmaClass : public TclClass {
public:
	OlsrTdmaClass() : TclClass("Olsr/Tdma") {}
	TclObject* create(int, const char*const*) {
		return (new OlsrTdma(&OMIB));
	}
} class_olsr_tdma;

// Olsr Tdma definitions
// Frame format:
// Pamble Slot1 Slot2 Slot3...
OlsrTdma::OlsrTdma(OLSR_MIB* p) : 
	OLSR(),mhSlot_(this), mhTxPkt_(this), mhRxPkt_(this){
	...
	*/
	...
	tdma_preamble_[slot_num_] = NOTHING_TO_SEND;
        printf("Slot_Count :%f\n",slot_count_);
	printf("tdma_preamble_[slot_num_] :%f\n",tdma_preamble_[slot_num_]);
	//Start the Slot timer..
	mhSlot_.start((Packet *) (& intr_), 0);  
}

/* similar to 802.11, no cached node lookup. */
int OlsrTdma::command(int argc, const char*const* argv)
{
	if (argc == 3) {
...
void OlsrTdma::recv(Packet* p, Handler* h) {
	...
	*/
	...
	state(MAC_SEND);
	sendDown(p);
	printf("<%d> packet sent down: %d\n", index_, tdma_ps_++); //Nitish
}

void OlsrTdma::sendUp(Packet* p) 
{
	struct hdr_cmn *ch = HDR_CMN(p);
...
void OlsrTdma::recvDATA(Packet *p){
	/*Adjust the OLSR packet size: strip off the olsr header.*/
	struct hdr_cmn *ch = HDR_CMN(p);
...
void OlsrTdma::sendDown(Packet* p) {
	struct hdr_cmn* ch = HDR_CMN(p);
	struct hdr_mac_tdma* dh = HDR_MAC_TDMA(p);

	/* Update the OLSR header, same as 802.11 */
	ch->size() += ETHER_HDR_LEN;
...
void OlsrTdma::radioSwitch(int i) 
{
	radio_active_ = i;
...
*/
void OlsrTdma::slotHandler(Event *e) 
{
	...
	if ((tdma_preamble_[slot_count_] == index_) || ((u_int32_t)tdma_preamble_[slot_count_] == MAC_BROADCAST)) {
		printf("<%d>, %f, preamble[%d]=%d, I am supposed to receive now.\n", index_, NOW, slot_count_, tdma_preamble_[slot_count_]);  //Nitish
		slot_count_++;
...
void OlsrTdma::recvHandler(Event *) 
{
	u_int32_t dst;
...
/* Not Required */
void OlsrTdma::sendHandler(Event *e) 
{
